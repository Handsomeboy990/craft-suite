...
export async function POST(request: Request) {
  ...
  const definitions = content.quote.fields;
  ...
  const message = addMessage('devis', fields);

  // The message is stored before anything is announced, so a failing
  // notification never loses it.
  const first = fields[0]?.value ?? '';
  void notify('Nouvelle demande de devis', first.slice(0, 120), '/admin/messages').catch((error) => {
