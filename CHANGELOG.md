...
Session 29, the animation that never ran, version 3.22.0:

- Looking at the running site found five gaps: the password could only be
  changed with a shell on the server, the favicon could not be changed at all,
  no animation was visible, the navigation was a wrapping row rather than a
  menu, and none of it was shaped by the trade.
- The animation one was a defect, not a preference. `.stagger > *` selected the
  list, not its items, so the delay landed on a container that had no transition
  and nothing ever moved in sequence. Everything else in the system worked, which
  is why it passed a reading and failed a look.
- `theme.motion.signature` now names which effects exist, after the trade, and
  the intensity only scales them. Verified both ways: the energetic instance
  runs entrance, stagger, counters and parallax; the technical one runs none of
  them and keeps the reveal and the lift. A signature that changes nothing would
  be a label, so the proof of absence is part of the suite.
- The password changes from the back office, the favicon is content, and the
  navigation is a menu below the breakpoint with the keyboard behaviour that
  goes with it.
- The lesson to carry: a system can be complete, typed, documented and dead. The
  stagger had a token, a CSS rule, a component and a resource file describing it,
  and it had never once run. Look at the thing.
